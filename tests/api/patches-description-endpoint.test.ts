...
// Create the mock for KV operations
jest.mock('@/lib/kv', () => {
  return {
    saveToKV: jest.fn().mockImplementation(() => Promise.resolve()),
    ...
    listKeysFromKV: jest.fn().mockImplementation(() => 
      Promise.resolve([
        'gear:test-gear-1',
        'gear:test-gear-2'
      ])
...
describe('Patch Description Endpoint', () => {
  ...
  it('should generate a description for a patch', async () => {
    // Mock the direct fetch response instead of mocking the AI SDK
    ...
    global.fetch = jest.fn().mockImplementation((): Promise<Response> => 
      Promise.resolve({
        ...
        text: async () => 'Test patch description generated by the model',
        headers: new Headers(),
        url: '',
        redirected: false,
        statusText: '',
